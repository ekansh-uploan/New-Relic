...
run() {
  ...
  if [ "${HIDE_CMD_LINE:-}" != 1 ] && [ "${HIDE_CMD_LINE:-}" != true ]; then
    echo exec $(exec_args) java -javaagent:/deployments/uploan-ph/newrelic.jar $(java_options) -cp "$(classpath)" ${args} "$@"
  fi
  ...
  # supported anyway.
  exec $(exec_args) java -javaagent:/deployments/uploan-ph/newrelic.jar $(java_options) -cp "$(classpath)" ${args} "$@"
}
